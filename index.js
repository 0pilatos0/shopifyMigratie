        ...
        item.data.forEach((connector) => {
            newConnectors.forEach(element => {
                if (element.__temp_id === connector.usersettings_id) {
                    ...
                    let isUseSerialnumbers = connector.use_serialnumbers === 'on' ? '1' : '0';
                    let isUsePrices = connector.use_prices === 'on' ? '1' : '0';
                    let isNotifyShipment = connector.notify_shipment === 'on' ? '1' : '0';
                    let isUsePendingPaymentOrders = connector.use_payment_pending_orders === 'on' ? '1' : '0';
                    let isUseOnlyLocationOrders = connector.use_only_location_orders === 'on' ? '1' : '0';

                    element.metadata = `a:11:{s:4:"shop";s:${element.name.length}:"${element.name}";s:11:"accessToken";s:${element.__temp_access_token.length}:"${element.__temp_access_token}";s:9:"webshopId";s:${connector.webshop_id.toString().length}:"${connector.webshop_id}";s:10:"locationId";i:${connector.location};s:14:"isUseStockSync";b:${isUseStockSync};s:16:"isUseShippingTag";b:${isUseShippingTag};s:18:"isUseSerialnumbers";b:${isUseSerialnumbers};s:11:"isUsePrices";b:${isUsePrices};s:16:"isNotifyShipment";b:${isNotifyShipment};s:25:"isUsePendingPaymentOrders";b:${isUsePendingPaymentOrders};s:23:"isUseOnlyLocationOrders";b:${isUseOnlyLocationOrders};}`
